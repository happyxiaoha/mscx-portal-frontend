...
// 上传图片
var uploadImgUrl = mscxPage.request.saas + 'pic/upload.do';
...
var addModel = Backbone.Model.extend({
    idAttribute: 'addId',
    url: mscxPage.request.saas + 'apply.do'
});
var modifyModel = Backbone.Model.extend({
    idAttribute: 'modifyId',
    url: mscxPage.request.saas + 'modify.do'
});

var checkUnique = Backbone.Model.extend({
    url: mscxPage.request.saas + 'checkUnique.do'
});
...
var createDemandView = Backbone.View.extend({
    ...
    changeChargeType: function (e) {
        ...
        this.chargeType = sId;
        (sId == '01' || sId == '03') ?  $('.server-package').hide() : ($('.server-package').show(),this.buildChargeTable());
        e.stopPropagation();
    },
